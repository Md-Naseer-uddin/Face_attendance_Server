...
app.post("/api/login", async (req, res) => {
  ...
  const result = await pool.query(`SELECT * FROM people WHERE email = $1`, [
    email,
  ]);

  if (email === result.email && password === result.password) {
    // Generate JWT token
    const token = jwt.sign({ email: result.email, role: result.role }, JWT_SECRET, {
      expiresIn: "24h",
    });
...
app.post("/api/register", authenticateToken,async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      faceCheck.rows[0].distance < FACE_SIMILARITY_THRESHOLD
    ) {
      return res.status(409).json({
        success: false,
...
app.post("/api/mark-attendance", authenticateToken,async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Liveness score must be between 0 and 1" });
  }
  ...
  try {
    ...
    const DISTANCE_THRESHOLD =
      parseFloat(process.env.DISTANCE_THRESHOLD) || 0.5;
    ...
    console.log(
      `✓ Attendance: ${match.name} (distance: ${match.distance.toFixed(
        3
